...
def parse_us_house():
    url = "http://electionstats.state.ma.us/elections/search/year_from:2016/year_to:2016/office_id:5/stage:General"
    r = requests.get(url)
    soup = BeautifulSoup(r.text)
...
def parse_state_senate():
    url = "http://electionstats.state.ma.us/elections/search/year_from:2016/year_to:2016/office_id:9/stage:General"
    r = requests.get(url)
    ...
    with requests.Session() as s:
        for link in election_links:
            ...
            for row in reader:
                cols = [x for x in row.keys() if x not in ['City/Town', 'Ward', 'Pct']]
                for col in cols:
                    if col == u'Sonia Rosa Chang-D\xcdaz':
                        party = 'Democratic'
                    else:
                        party = final_cands[col]
